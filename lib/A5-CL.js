...
a5.Package('a5.cl.mixins')
	.Mixin('BindableSource', function(mixin, im){
		...
		mixin.notifyReceivers = function(param){
			if(typeof param === 'function')
				this._cl_notifyReceiversFromParamCallback(param);
			else
				this._cl_notifyReceivers(param);
		}
		...
		mixin._cl_notifyReceivers = function(data){
		...
		mixin._cl_notifyReceiversFromParamCallback = function(callback){
			for (var i = 0, l = this._cl_receivers.length; i < l; i++) {
				var r = this._cl_receivers[i];
			...
			for(var i = 0, l = this._cl_bindings.length; i<l; i++){
				...
				if (b.persist !== true) {
					if(value)
						...
						b.source._cl_detachReceiver(b.receiver);	
				}
			}	
